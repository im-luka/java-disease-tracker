...
public class DodavanjeVirusaController implements Initializable {
    ...
    public void dodajVirus() throws IOException {
        try {
            ...
            Integer velicina = PretragaVirusaController.getObservableListaVirusa().size() + 1;

            Virus vir = new Virus((long)velicina, naziv, true, pomocnaListaSimptoma);

            PretragaBolestiController.getObservableListaBolesti().add(vir);
            PretragaVirusaController.getObservableListaVirusa().add(vir);
            BazaPodataka.spremiBolestUBazu(vir, true);
