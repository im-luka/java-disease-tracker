...
public class PocetniEkranController implements Initializable {
    ...
    /**
     * Inicijalizira cijelu aplikaciju s podacima iz baze podataka kako bi korisnici preko korisničkog sučelja aplikacije mogli
     * dohvaćati, pretraživati i dodavati nove županije/simptome/bolesti/viruse/osobe.
     *
     * @param url
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        if(PretragaZupanijaController.getObservableListaZupanija() == null){
            ...
            try {
                BazaPodataka.dohvatiZupanije(zupanijeLista);
            }
            catch (SQLException | IOException iznimka) {
                logger.error("Greška prilikom dohvata podataka o županijama iz baze podataka! ", iznimka);
                iznimka.printStackTrace();
            }
            PretragaZupanijaController.setObservableListaZupanija(FXCollections.observableArrayList(zupanijeLista));
        }

        if(PretragaSimptomaController.getObservableListaSimptoma() == null){
            List<Simptom> simptomiLista = new ArrayList<>();
            try {
                BazaPodataka.dohvatiSimptome(simptomiLista);
            }
            catch (SQLException | IOException iznimka) {
                logger.error("Greška prilikom dohvata podataka o simptomima iz baze podataka! ", iznimka);
                iznimka.printStackTrace();
            }
            PretragaSimptomaController.setObservableListaSimptoma(FXCollections.observableArrayList(simptomiLista));
        }

        if(PretragaBolestiController.getObservableListaBolesti() == null && PretragaVirusaController.getObservableListaVirusa() == null){
            List<Bolest> bolestiLista = new ArrayList<>();
            List<Virus> virusiLista = new ArrayList<>();
            try {
                BazaPodataka.dohvatiBolestVirus(bolestiLista, virusiLista);
            }
            catch (SQLException | IOException iznimka) {
                logger.error("Greška prilikom dohvata podataka o bolestima/virusima iz baze podataka! ", iznimka);
                iznimka.printStackTrace();
            }
            PretragaBolestiController.setObservableListaBolesti(FXCollections.observableArrayList(bolestiLista));
            PretragaVirusaController.setObservableListaVirusa(FXCollections.observableArrayList(virusiLista));
        }

        if(PretragaOsobaController.getObservableListaOsoba() == null){
            List<Osoba> osobeLista = new ArrayList<>();
            try {
                BazaPodataka.dohvatiOsobe(osobeLista);
            }
            catch (SQLException | IOException iznimka) {
                logger.error("Greška prilikom dohvata podataka o osobama iz baze podataka! ", iznimka);
                iznimka.printStackTrace();
            }
            PretragaOsobaController.setObservableListaOsoba(FXCollections.observableArrayList(osobeLista));
        }
    }
