...
import main.java.hr.java.covidportal.niti.OsobeDodajNit;
...
public class DodavanjeOsobeController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        try {
            tablicaKontaktiranihOsobe.setItems(FXCollections.observableArrayList(BazaPodataka.dohvatiOsobe()));
            zupanijaOsobe.setItems(FXCollections.observableArrayList(BazaPodataka.dohvatiZupanije()));
            bolestOsobe.setItems(FXCollections.observableArrayList(BazaPodataka.dohvatiBolestVirus()));
        } catch (SQLException | IOException iznimka) {
            iznimka.printStackTrace();
            PocetniEkranController.logger.error("Greška prilikom dohvata podataka o kontaktiranim osobama, županiji ili bolesti iz baze podataka! ", iznimka);
        }
    ...
    public void dodajOsobu() throws IOException {
        try {
            ...
            Integer velicina = BazaPodataka.dohvatiOsobe().size() + 1;
            ...
            servis.execute(new OsobeDodajNit(os));
